...
payload="$(cat <<END
{
    ...
    "body": "Automated [build]($build_url) (#$DRONE_BUILD_NUMBER) of LLVM + Clang as of commit [$short_llvm_commit]($llvm_commit_url) and binutils as of commit [$short_binutils_commit]($binutils_commit_url).",
    "prerelease": true
}
END
)"
